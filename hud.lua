...
spawnit.hud = futil.define_hud("spawnit:hud", {
	...
	get_hud_def = function(player, stats)
		local lines = {
			...
			f("#cached_blocks = %i", stats.cached_blocks),
			f("ao_calc_usage = %ius/s", stats.ao_calc_usage),
			f("get_ao_blocks = %.1f%%", stats.get_ao_blocks_usage * 100 / stats.ao_calc_usage),
			f("async_callback_usage = %ius/s", stats.async_callback_usage),
			f("spawn_mobs_usage = %ius/s", stats.spawn_mobs_usage),
		}
